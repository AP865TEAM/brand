...
const meanings = ['프라이버시 · 존중', '정확성 · 자연스러움 · 신뢰', '통합 관리 · 지속성 · 개인화', '조용한 고급감 · 경험의 완성도'];
const CYCLE = 5;
const positions = sourceValues.map((value, index) => {
  const group = keywords.findIndex(item => item.sources.includes(value.id));
  return { x: 25 + Math.floor(index / 5) * 50, y: 10 + (index % 5) * 19, targetX: 25 + (group % 2) * 50, targetY: 29 + Math.floor(group / 2) * 48 };
});
...
export default function ValueConvergence() {
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      sequence.push(
        ...
        ['.caption-four', { opacity: [0, 0, 1, 1, 0] }, { at: 0, duration: CYCLE, times: [0, .4, .56, .86, 1] }],
      );
  ...
  return <div className={`convergence editorial-motion ${ready ? 'is-ready' : ''} ${reduced ? 'is-reduced' : ''}`} ref={scope}>
    <div className="motion-topline"><span>AP865 / The essence</span><span>10 perspectives → 4 core values</span></div>
    <div className="motion-layout">
      <div className="motion-art" aria-hidden="true">
        <div className="editorial-caption"><span className="caption-ten">열 가지 기준.</span><span className="caption-four">네 가지 핵심 가치.</span></div>
        <div className="type-stage" ref={stage}>
          ...
          {keywords.map((item, index) => <div className="keyword-anchor" key={item.keyword} style={{ left: `${25 + (index % 2) * 50}%`, top: `${29 + Math.floor(index / 2) * 48}%` }}><div className="keyword-reveal"><h3>{item.keyword}</h3><p>{meanings[index]}</p><span>{item.sources.join(' / ')}</span></div></div>)}
        </div>
      </div>
      <aside className="motion-reading" aria-label="네 코어밸류의 도출 근거">
        <span>From meaning to identity</span>
        <p>가치의 중심을 따라,<br />네 가지로 정의합니다.</p>
    ...
    <div className="motion-controls"><p>{reduced ? '동작 줄이기 설정 · 네 코어밸류 요약' : '10 → 4 · 5초 자동 반복'}</p>{!reduced && <div><button type="button" onClick={() => setPaused(value => !value)}>{paused ? '재생' : '일시정지'}</button><button type="button" onClick={restart}>다시 보기 ↻</button></div>}</div>
  </div>;
}
