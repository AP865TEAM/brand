  ...
  return (
    <main id="top">
      ...
      <header className="site-header">
        <a className="brand-link" href="#top" aria-label="AP865 맨 위로">AP865</a>
        <nav aria-label="브랜드 페이지">
          <a href="#core-values">01 코어밸류</a><a href="#color">02 키 컬러</a><a href="#proposals">로고 시안</a>
        </nav>
        <span className="edition">{foundation.status}</span>
      </header>

      <section className="foundation-page values-page" id="core-values" aria-labelledby="values-title">
        <figure className="opening-visual">
          ...
          <figcaption><span>AP865 / Brand direction</span><h1 id="values-title">{foundation.title}</h1><div><p>Core values, before identity.</p><span>공간 무드 이미지 · 실제 시공 이미지 아님</span></div></figcaption>
        </figure>
        <PageLabel number="01" title="Core Values / 10 → 4" />
        <div className="page-intro">
          <div><span className="eyebrow">Defining our essence</span><h2>열 가지 기준에서,<br />네 가지 핵심 가치로.</h2></div>
          <div className="foundation-introduction"><p>{foundation.introduction}</p><p>이 열 가지 가치를 의미별로 정의하면 네 가지로 요약됩니다. AP865는 Private · Skin · Concierge · Premium을 핵심 키워드이자 코어밸류로 삼습니다.</p></div>
        </div>
        <ValueConvergence />
        <div className="core-conclusion" id="keywords"><span className="eyebrow">Our four core values</span><h2>우리가 지킬 약속,<br />브랜드를 이끄는 네 가지.</h2><p>Private · Skin · Concierge · Premium</p></div>
        ...
        <details className="source-principles"><summary>아름다움에 대한 열 가지 기준 · 원문과 연결 보기 <span>＋</span></summary><p className="grouping-note">모션은 각 기준의 중심 의미를 따라 한 그룹에 연결합니다. 네 코어밸류는 실제 브랜드 경험에서 서로 맞물려 작동합니다.</p><ol className="value-list">
        ...
        <div className="page-end"><span>네 가지 핵심 가치에서 브랜드의 시각 언어로.</span><a href="#color">키 컬러와 방향성 ↓</a></div>
      </section>

      <section className="foundation-page color-page" id="color" aria-labelledby="color-title">
        <PageLabel number="02" title="Key Color & Direction" />
        <div className="page-intro">
        ...
        <div className="selection-criteria"><div><span className="eyebrow">Criteria for identity</span><h3>로고는 이 방향을<br />어떻게 증명하는가.</h3></div><ol>{designCriteria.map((item, index) => <li key={item.title}><span>0{index + 1}</span><div><h4>{item.title}</h4><p>{item.description}</p></div></li>)}</ol></div>
      ...
      <section className="proposal-section" id="proposals" aria-labelledby="proposals-title">
        <div className="proposal-heading"><span className="eyebrow">Next / Brand Identity</span><h2 id="proposals-title">Logo Proposals</h2><p>브랜드 방향성을 형태로 제안하는 단계.<br />시안 전달 후, 각 제안의 의도와 근거를 이곳에 담습니다.</p></div>
        <div className="proposal-slots">
          {logoProposals.map(proposal => <article className={proposal.asset ? 'proposal-slot is-ready' : 'proposal-slot'} key={proposal.id}>
